...
export default function RevenueChart({ data, timePeriod }: RevenueChartProps) {
  ...
  const formatCurrency = (value: number) => `LKR ${(value / 1000).toFixed(1)}k`;

  const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number; dataKey: string }>; label?: string }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-[var(--bg-card)] border border-[var(--border-hover)] rounded-lg px-4 py-3 shadow-lg">
          <p className="text-xs text-[var(--text-label)] mb-1">{label}</p>
